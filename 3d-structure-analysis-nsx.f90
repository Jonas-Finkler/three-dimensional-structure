...
program orderparameter 
  ...
  integer :: ntype(100),ncells(3), d(3), d0(3), d1(3),n_neighbor,i0,k0, nn1,nn2,numN,&
  & nshells, n_fib !=4610 !nb of points be to evently spaced on the sphere  
  ...
  !  corr=3
  !endif
  read(correlation(1:1), *) el1 ! central atom (Si)
  read(correlation(2:2), *) el2 ! to construct reference frame (O)
  read(correlation(3:3), *) el3 ! what is plotted 

  print*, 'Element selection:', el1, el2, el3

  !---------read the config file, in xyz format------------------------
  !open(unit=12, file='config-nsx.xyz',status="old",action="read",iostat=ierror)
  open(unit=12, file='input_file.xyz',status="old",action="read",iostat=ierror)
  ...
  80 continue 
  write(*,*) "n_atom of different types= ", ntype(el1), ntype(el2), ntype(el3)
  close(12)   
  ...
  shells: do nshell=1,n_shell
    ...
    do i=1,nfrac !nline
      if (typat(i)==el1) then !type_2 as ref, i.e. particle #1 
          ...
          do j0=d(2)-nn1,d(2)+nn1
            do k0=d(3)-nn1,d(3)+nn1
              ...
              do k=1,nb(d1(1),d1(2),d1(3))
                if(int(codcell(d1(1),d1(2),d1(3),k,5))==el2) then !atype_1
        ...
        do i0=d(1)-nn1,d(1)+nn1 !search cells only within the outer bounds
          do j0=d(2)-nn1,d(2)+nn1
            do k0=d(3)-nn1,d(3)+nn1
              ...
              do k=1,nb(d1(1),d1(2),d1(3))
                if(int(codcell(d1(1),d1(2),d1(3),k,5))==el3) then   !atyp_corr as target particles 
                  delta0(:)= codcell(d1(1),d1(2),d1(3),k,1:3)-wrapped(i,:)
                  delta0(:)= delta0(:)-lbox(:)*anint(delta0(:)/lbox(:))  !PBC in 3D
            ...
            if (ana=="sff2") then
              do j=1,numN
                do k=1,n_area
                  ...
                  ! NOTE: Some atoms might be missed or double counted? Should be fine because we consider density only
    ...
    !--------sff2----------------------------------------
    if (ana=="sff2") then
      if (sum(npoints) > 0) then
        call system ('mkdir -p structure')
        ! JF: Changed file name to be ovito compatible
        ! write (seqstring,'(i0.2,f3.2)') int(rad),rad-int(rad)
        !filename0='spherical-density-r'//trim(seqstring)//'-nsx-corr'//trim(correlation)//'.xyz'
        write (seqstring,'(i0.4)') int(rad * 100)      
        filename0='spherical-density-nsx-corr' // trim(correlation) // '-r' //trim(seqstring)//'.xyz'
