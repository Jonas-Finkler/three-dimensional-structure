...
!
!this code performs 3d structural order analysis for the case of Na2O-SiO2 system
!note that this system has a well-defined local structure which is SiO4 tetrahedron
!The configuration by default is in 3d-periodic, but non-PBC configs can easily 
!be adapted to this code
!
...
!
!by Zhen Zhang, zhen.zhang1991@hotmail.com, May 21, 2022
!
...
!~    USE MPI
   ...
   real(kind=8), parameter :: min1_gr=1.4  !the first min of gr: 2.0 for SiO2, 1.4 for BLJM
   ...
   real(kind=8) :: dr, rmax, rmin, d_area,tmp0,tmp2(3),delta0(3),vect_z(3),&
   ...
   ana=args(1) !sff1
   correlation=args(2) !11 -> AA
   read(args(3),'(f5.2)')minr !2.0
   read(args(4),'(f5.2)')maxr !15.0
   read(args(5),'(f5.2)')dr  !0.05, define the resoultion of the sff curve
   read(args(6),'(f5.2)')delta  !0.5, the thickness of the layer
   !read(args(7),'(f5.2)')theta_del  !3.0, for 4610 cells on the sphere
   read(args(7),'(i6)') n_fib  !nb of cells on the sphere
   read(args(8),'(i3)') l_value  !3 for SiO2, 6 for BLJM, the mode of SH to be calculated
   read(args(9),'(f5.3)')frac !fraction of the config to be analyzed
   
   !-----------------------------------------------------------------
   if(correlation=="11") then  !atyp1_atyp1
      corr=1 
   elseif(correlation=="12") then  !atyp1_atyp2
      corr=2
   elseif(correlation=="13") then  !atyp1_atypN
      corr=3
   endif
   
   !---------read the config file, in xyz format------------------------
   open(unit=12, file='config-bljm.xyz',status="old",action="read",iostat=ierror)
...
!---------create files to write for sff1 analysis-----------
   ...
   filename4='Srho-l'//trim(seqstring)//'-bljm-corr'//trim(correlation)//'.dat' !S_rho
...
!------start the algorithm for 3d structural order----
  ...
  do i=1,nfrac !nline
   if (typat(i)==1) then !type_1 as ref, i.e. particle #1 
   ...
   do k0=d(3)-2,d(3)+2
	...
	do k=1,nb(d1(1),d1(2),d1(3))
	 ...
	 if (norm2(delta0)>0.01 .and. norm2(delta0)<=min1_gr) then  !typ1_typ1 
...
!~ 	  endif
    ...
    do n1=1,n_neighbor-1   
     do n2=n1+1,n_neighbor
      delta0(:)=vect_neighbor(n1,:)-vect_neighbor(n2,:)
      delta0(:)= delta0(:)-lbox(:)*anint(delta0(:)/lbox(:))  !PBC in 3D
      distance=norm2(delta0)
      if (distance>0.01 .and. distance<=min1_gr) then !the triplet are nearest neighbor to each other
    ...
    endif !the triplet are nearest neighbor to each other
     enddo !n2
    enddo  !n1
  ...
  filename0='spherical-density-r'//trim(seqstring)//'-bljm-corr'//trim(correlation)//'.xyz'
